...
// Llista tasques: un treballador veu les que té assignades i les del reté (si ho és ell)
...
// Crear tasca (només encarregats) — es pot assignar a un o més usuaris, i/o al reté
router.post('/', requireEncarregat, async (req: AuthRequest, res) => {
  const { titol, descripcio, assignatsAIds, assignatAlReten, dataLimit, prioritat, repeticio } = req.body;
  const ids: string[] = Array.isArray(assignatsAIds) ? assignatsAIds : [];
  if (ids.length === 0 && !assignatAlReten) {
    return res.status(400).json({ error: 'Cal assignar la tasca a algú o al reté' });
  }
...
// Canviar estat d'una tasca (un dels assignats, el reté si li pertoca, o un encarregat)
router.patch('/:id/estat', async (req: AuthRequest, res) => {
  ...
  if (req.usuari!.rol === 'TREBALLADOR' && !esAssignat && !potPerReten) {
